...
async function verify2012Data() {
  const sampleDates = [
    '2012-01-01',
    '2012-02-29', // Jour bissextile crucial
    '2012-04-15',
    '2012-06-21',
    '2012-08-10',
    '2012-10-25',
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2012 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2012');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2012');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2012: ${dailyCount} daily, ${hourlyCount} hourly (attendu: 366 chacun pour une année bissextile)`);
  if (dailyCount !== 366 || hourlyCount !== 366) {
    throw new Error(`Comptage incomplet: daily=${dailyCount}, hourly=${hourlyCount} (attendu 366)`);
  }

  // Vérifier la présence explicite du 29 février 2012
  const leapDayDaily = path.join(DATA_DIR, 'daily', '2012', '02', '29.json');
  const leapDayHourly = path.join(DATA_DIR, 'hourly', '2012', '02', '29.json');
  if (!fs.existsSync(leapDayDaily) || !fs.existsSync(leapDayHourly)) {
    throw new Error("Le jour bissextile 2012-02-29 est manquant !");
  }
  console.log(`[OK] Jour bissextile 2012-02-29 bien présent.`);
