...
const YEAR = 2004;
const EXPECTED_DAYS = 366; // 2004 est une année bissextile (29 jours en février)

const DATA_DIR = path.join(process.cwd(), 'data');

async function fetchAndSaveYear2004() {
  ...
  if (dates.length !== EXPECTED_DAYS) {
    throw new Error(`Année ${YEAR} incomplète: ${dates.length} jours reçus (attendu ${EXPECTED_DAYS} pour une année bissextile).`);
  }
  if (hourly.time.length !== EXPECTED_DAYS * 24) {
    throw new Error(`Heures ${YEAR} incomplètes: ${hourly.time.length} (attendu ${EXPECTED_DAYS * 24}).`);
  }
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    // Stockage exact sans altération ni arrondi des données brutes reçues de l'API
    const dayData = {
      date,
...
fetchAndSaveYear2004().catch(err => {
