...
const YEAR = 2001;
const EXPECTED_DAYS = 365; // 2001 est une année standard (non bissextile, 28 jours en février)

const DATA_DIR = path.join(process.cwd(), 'data');

async function verify2001Data() {
  const sampleDates = [
    '2001-01-01',
    '2001-02-14',
    '2001-03-21',
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2001 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2001');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2001');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2001: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
