...
async function verify2008Data() {
  const sampleDates = [
    '2008-01-01',
    '2008-02-14',
    '2008-02-29', // Jour bissextile crucial
    '2008-03-21',
    '2008-05-15',
    '2008-07-20',
    '2008-08-15',
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2008 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2008');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2008');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2008: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2008 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
