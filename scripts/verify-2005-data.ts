...
async function verify2005Data() {
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2005 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2005');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2005');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2005: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2005 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
