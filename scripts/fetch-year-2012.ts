...
const YEAR = 2012;
const EXPECTED_DAYS = 366; // 2012 est une année bissextile (29 jours en février)

const DATA_DIR = path.join(process.cwd(), 'data');

async function fetchAndSaveYear2012() {
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    // Stockage exact sans arrondi des données brutes reçues de l'API
    const dayData = {
      date,
...
fetchAndSaveYear2012().catch(err => {
