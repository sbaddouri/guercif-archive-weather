...
async function verify2010Data() {
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2010 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2010');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2010');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2010: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2010 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
