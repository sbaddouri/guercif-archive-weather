...
async function verifyDates() {
  ...
  console.log(`=== VÉRIFICATION ${YEAR} vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', YEAR);
  const hourlyDir = path.join(DATA_DIR, 'hourly', YEAR);
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);
  console.log(`Fichiers locaux: ${dailyCount} daily, ${hourlyCount} hourly (attendu ${EXPECTED_DAYS} chacun)`);
