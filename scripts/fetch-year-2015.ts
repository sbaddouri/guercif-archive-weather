...
async function fetchAndSaveYear2015() {
  console.log(`[INFO] Récupération de l'année 2015 depuis Open-Meteo...`);
  console.log(`[INFO] Coordonnées: LAT=${LAT}, LON=${LON}, Modèle: ${MODEL}, Timezone: ${TIMEZONE}`);

  const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${LAT}&longitude=${LON}&start_date=2015-01-01&end_date=2015-12-31&hourly=temperature_2m,relative_humidity_2m,dew_point_2m,precipitation,weather_code,pressure_msl,wind_speed_10m,wind_gusts_10m,visibility,uv_index,sunshine_duration&daily=weather_code,temperature_2m_max,temperature_2m_min,temperature_2m_mean,precipitation_sum,sunshine_duration,wind_speed_10m_max,sunrise,sunset&timezone=${TIMEZONE}&models=${MODEL}`;
  ...
  // 1. Organiser les données horaires par date
  ...
  // 2. Traiter et enregistrer chaque jour
  let savedDaily = 0;
  let savedHourly = 0;
  ...
  console.log(`[SUCCESS] Année 2015 complétée : ${savedDaily} fichiers journaliers et ${savedHourly} fichiers horaires enregistrés.`);
}

fetchAndSaveYear2015().catch(err => {
