...
const YEAR = 2011;
const EXPECTED_DAYS = 365; // 2011 est une année standard (non bissextile, 28 jours en février)

const DATA_DIR = path.join(process.cwd(), 'data');

async function fetchAndSaveYear2011() {
  ...
  if (dates.length !== EXPECTED_DAYS) {
    throw new Error(`Année ${YEAR} incomplète: ${dates.length} jours reçus (attendu ${EXPECTED_DAYS}).`);
  }
...
fetchAndSaveYear2011().catch(err => {
