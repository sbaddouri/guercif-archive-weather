...
async function verify2003Data() {
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2003 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2003');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2003');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2003: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2003 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
