...
async function verify2007Data() {
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2007 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2007');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2007');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2007: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2007 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
