...
async function verify2004Data() {
  const sampleDates = [
    '2004-01-01',
    '2004-02-14',
    '2004-02-29', // Jour bissextile crucial
    '2004-03-21',
    '2004-05-15',
    '2004-07-20',
    '2004-08-15',
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2004 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2004');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2004');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2004: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2004 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
