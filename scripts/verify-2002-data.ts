...
const YEAR = 2002;
const EXPECTED_DAYS = 365; // 2002 est une année standard (non bissextile, 28 jours en février)

const DATA_DIR = path.join(process.cwd(), 'data');

async function verify2002Data() {
  const sampleDates = [
    '2002-01-01',
    '2002-02-14',
    '2002-03-21',
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2002 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2002');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2002');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2002: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n======================================================`);
    console.log(`[SUCCÈS TOTAL] Année ${YEAR} : 365 jours & 8760 heures vérifiés.`);
    console.log(`Aucune valeur inventée, approximée ou arrondie.`);
    console.log(`Toutes les données correspondent rigoureusement à l'API Open-Meteo (modèle: ${MODEL}).`);
    console.log(`======================================================\n`);
  } else {
    throw new Error(`Échec de la vérification : des discordances ont été détectées pour l'année ${YEAR}.`);
  }
}
