...
async function verify2006Data() {
  ...
  console.log(`=== VÉRIFICATION RIGOUREUSE 2006 vs API OPEN-METEO (best_match) ===\n`);

  const dailyDir = path.join(DATA_DIR, 'daily', '2006');
  const hourlyDir = path.join(DATA_DIR, 'hourly', '2006');
  const dailyCount = countJsonFiles(dailyDir);
  const hourlyCount = countJsonFiles(hourlyDir);

  console.log(`Fichiers locaux détectés pour 2006: ${dailyCount} daily, ${hourlyCount} hourly (attendu: ${EXPECTED_DAYS} chacun)`);
  ...
  if (allMatches) {
    console.log(`\n✅ SUCCÈS TOTAL : Toutes les vérifications pour 2006 sont validées avec succès. Aucune valeur n'a été altérée, arrondie ou inventée.`);
  } else {
